...
export function ProfessorSignupForm() {
  ...
  const signupMutation = useMutation({
    mutationFn: () =>
      signupProfessor({ email, password, firstName, lastName, title, department }),
    onSuccess: () => {
      pushToast({ tone: "success", title: "Account created", description: "You can now create assignments." });
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    const nextErrors = validateSignupForm({ firstName, lastName, email, password, confirmPassword });
  ...
  return (
    <section className="auth-card auth-card-signup fade-up">
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "0.5rem", textAlign: "center" }}>
        <div style={{
          width: 44, height: 40, borderRadius: "10px",
          background: "var(--brand)", display: "grid", placeItems: "center", color: "#fff",
        }}>
          <BookOpen size={20} strokeWidth={2} />
        </div>
        <div>
          <p className="eyebrow" style={{ marginBottom: "0.18rem" }}>Instructor Access</p>
          <h1 style={{ fontSize: "1.34rem", fontWeight: 800, letterSpacing: "-0.025em", margin: 0 }}>
            Create Account
          </h1>
      ...
      {/* Form */}
      <form noValidate onSubmit={handleSubmit} style={{ display: "grid", gap: "0.65rem" }}>
        {attemptedSubmit && Object.values(fieldErrors).some(Boolean) ? (
          <div className="validation-summary" role="alert">
            <strong>Please review the account details below.</strong>
            <ul>
              {Object.values(fieldErrors).filter(Boolean).map((message) => <li key={message}>{message}</li>)}
            </ul>
          </div>
        ) : null}
          ...
          <div className="field">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (attemptedSubmit) {
                  setFieldErrors(validateSignupForm({ firstName: nextValue, lastName, email, password, confirmPassword }));
                }
              }}
              autoComplete="given-name"
              placeholder="e.g. Sarah"
            />
            {fieldErrors.firstName ? <p className="field-error" id="signup-first-name-error">{fieldErrors.firstName}</p> : null}
          </div>
          <div className="field">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (attemptedSubmit) {
                  setFieldErrors(validateSignupForm({ firstName, lastName: nextValue, email, password, confirmPassword }));
                }
              }}
              autoComplete="family-name"
              placeholder="e.g. Chen"
            />
            {fieldErrors.lastName ? <p className="field-error" id="signup-last-name-error">{fieldErrors.lastName}</p> : null}
          </div>
        </div>
        ...
        <div className="split-grid">
          ...
          <div className="field">
            <label className="field-label" htmlFor="department">
              Department <span style={{ color: "var(--text-muted)", fontWeight: 400 }}>(optional)</span>
            </label>
            <input
              id="department"
        ...
        <div className="field">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setEmail(nextValue);
              if (attemptedSubmit) {
                setFieldErrors(validateSignupForm({ firstName, lastName, email: nextValue, password, confirmPassword }));
              }
            }}
            type="email"
            autoComplete="email"
          ...
          />
          {fieldErrors.email ? <p className="field-error" id="signup-email-error">{fieldErrors.email}</p> : null}
        </div>
        ...
        <div className="field">
          ...
          <div className="input-wrap">
            <input
              ...
              onChange={(e) => {
                ...
                if (attemptedSubmit) {
                  setFieldErrors(validateSignupForm({ firstName, lastName, email, password: nextValue, confirmPassword }));
                }
              }}
          ...
          {fieldErrors.password ? (
            <p className="field-error" id="signup-password-error">{fieldErrors.password}</p>
          ) : (
            <p className="field-hint" id="signup-password-hint">Use at least 8 characters. Longer passwords with mixed character types work best.</p>
          )}
        </div>
        ...
        <div className="field">
          ...
          <div className="input-wrap">
            <input
              id="confirm-password"
              aria-describedby={fieldErrors.confirmPassword ? "signup-confirm-password-error" : "signup-confirm-password-hint"}
              ...
              onChange={(e) => {
                ...
                if (attemptedSubmit) {
                  setFieldErrors(validateSignupForm({ firstName, lastName, email, password, confirmPassword: nextValue }));
                }
              }}
          ...
          {fieldErrors.confirmPassword ? (
            <p className="field-error" id="signup-confirm-password-error">{fieldErrors.confirmPassword}</p>
          ) : confirmPassword && password === confirmPassword ? (
            <p className="field-success" id="signup-confirm-password-hint">Passwords match.</p>
          ) : (
            <p className="field-hint" id="signup-confirm-password-hint">Re-enter the same password to confirm the account setup.</p>
          )}
        </div>
        ...
        <button
          ...
          style={{ minHeight: 42, fontSize: "0.92rem", marginTop: "0.15rem" }}
      ...
      {/* Footer link */}
      <p style={{ margin: 0, fontSize: "0.83rem", color: "var(--text-muted)", textAlign: "center" }}>
